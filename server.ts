...
import { IniDoc, parseIniDoc, toIniText, patchIniValues } from "./src/ini.ts";
import { discoverConfigs, resolveWithFallback, isSubPath } from "./src/discovery.ts";
...
async function handleSave(req: Request): Promise<Response> {
  ...
  if (schema) {
    ...
    if (!result.valid) return jsonResponse({ ok: false, errors: result.errors }, 422);
  }
...
async function handleBrowse(req: Request): Promise<Response> {
  ...
  try {
    for await (const e of Deno.readDir(abs)) {
      ...
      entries.push({ name: e.name, path: p, type: e.isDirectory ? "dir" : "file" });
    }
  } catch {
...
function route(req: Request): Promise<Response> | Response {
  ...
  if (req.method === "GET" && pathname === "/api/file") return handleGetFile(req);
  if (req.method === "POST" && pathname === "/api/save") return handleSave(req);
  if (req.method === "POST" && pathname === "/api/save-as") return handleSaveAs(req);
  if (req.method === "GET" && pathname === "/api/browse") return handleBrowse(req);

  // Serve client assets
  if (req.method === "GET" && (pathname === "/app.js" || pathname === "/style.css")) {
    const file = pathname === "/app.js" ? "./static/app.js" : "./static/style.css";
    return Deno.readTextFile(file)
      .then((text) =>
        new Response(text, {
          headers: { "content-type": contentType(pathname.split(".").pop() || "text") || "text/plain" },
        })
      )
